...
pub struct WhisperInferenceParams {
    ...
    /// Entropy threshold for hallucination detection.
    /// Segments with entropy below this value are rejected as potential hallucinations.
    /// Defaults to 2.4 in whisper.cpp. Set to 0.0 to disable the check entirely,
    /// which is necessary for scripts like Thaana (Dhivehi) whose multi-byte UTF-8
    /// tokens produce inherently low entropy.
